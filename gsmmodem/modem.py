...
class GsmModem(SerialComms):
    ...
    def __init__(self, port, baudrate=115200, incomingCallCallbackFunc=None, smsReceivedCallbackFunc=None,
                 cpinCallbackFunc=None,
                 smsStatusReportCallback=None, requestDelivery=True, AT_CNMI="", *a, **kw):
        ...
        self.cpinCallbackFunc = cpinCallbackFunc
    ...
    def connect(self, pin=None, waitingForModemToStartInSeconds=0):
        ...
        if enableWind:
            try:
                wind = lineStartingWith('+WIND:',
                                        self.write('AT+WIND?'))  # Check current WIND value; example response: +WIND: 63
            except CommandError:
    ...
    def write(self, data, waitForResponse=True, timeout=10, parseError=True, writeTerm=TERMINATOR,
        ...
        responseLines = super(GsmModem, self).write(data + writeTerm, waitForResponse=waitForResponse, timeout=timeout,
                                                    expectedResponseTermSeq=expectedResponseTermSeq)
                ...
                if 'ERROR' in cmdStatusLine:
                    ...
                    if cmErrorMatch:
                        ...
                        if errorCode == 515 or errorCode == 14:
                            ...
                            self.log.debug('Device/SIM busy error detected; self._writeWait adjusted to %fs',
                                           self._writeWait)
                            time.sleep(self._writeWait)
                            result = self.write(data, waitForResponse, timeout, parseError, writeTerm,
    ...
    def networkName(self):
        ...
        copsMatch = lineMatching('^\+COPS: (\d),(\d),"(.+)",{0,1}\d*$',
                                 self.write('AT+COPS?'))  # response format: +COPS: mode,format,"operator_name",x
        if copsMatch:
    ...
    def _compileSmsRegexes(self):
        ...
        if self.smsTextMode:
            if self.CMGR_SM_DELIVER_REGEX_TEXT == None:
                ...
                self.CMGR_SM_REPORT_REGEXT_TEXT = re.compile(
        ...
        try:
            ...
            if response is "OK":  # command is supported, but no number is set
                return None
                    ...
                    response) == 2:  # OK and phone number. Actual number is in the first line, second parameter, and is placed inside quotation marks
                cnumLine = response[0]
    ...
    def waitForNetworkCoverage(self, timeout=None):
        ...
        while block[0]:
            if checkCreg:
                cregResult = lineMatching('^\+CREG:\s*(\d),(\d)(,[^,]*,[^,]*)?$',
                                          self.write('AT+CREG?', parseError=False))  # example result: +CREG: 0,1
                if cregResult:
    ...
    def sendSms(self, destination, text, waitForDeliveryReport=False, deliveryTimeout=15, sendFlash=False):
        ...
        if self.smsTextMode:
            ...
            for pdu in pdus:
                ...
                result = lineStartingWith('+CMGS:',
                                          self.write(str(pdu), timeout=35, writeTerm=CTRLZ))  # example: +CMGS: xx
    ...
    def sendUssd(self, ussdString, responseTimeout=15):
        ...
        try:
            cusdResponse = self.write('AT+CUSD=1,"{0}",15'.format(ussdString),
                                      timeout=responseTimeout)  # Should respond with "OK"
        except Exception:
    ...
    def checkForwarding(self, querytype, responseTimeout=15):
        ...
        """
        try:
            queryResponse = self.write('AT+CCFC={0},2'.format(querytype),
                                       timeout=responseTimeout)  # Should respond with "OK"
        except Exception:
            raise
        print(queryResponse)
    ...
    def setForwarding(self, fwdType, fwdEnable, fwdNumber, responseTimeout=15):
        ...
        """
        try:
            queryResponse = self.write('AT+CCFC={0},{1},"{2}"'.format(fwdType, fwdEnable, fwdNumber),
                                       timeout=responseTimeout)  # Should respond with "OK"
        except Exception:
            raise
            return False
    ...
    def listStoredSms(self, status=Sms.STATUS_ALL, memory=None, delete=False):
        ...
        if self.smsTextMode:
            ...
            for line in result:
                ...
                if cmglMatch:
                    ...
                    if msgIndex != None and len(msgLines) > 0:
                        msgText = '\n'.join(msgLines)
                        msgLines = []
                        messages.append(ReceivedSms(self, Sms.TEXT_MODE_STATUS_MAP[msgStatus], number,
                                                    parseTextModeTimeStr(msgTime), msgText, None, [], msgIndex))
            ...
            if msgIndex != None and len(msgLines) > 0:
                ...
                msgLines = []
                messages.append(
                    ReceivedSms(self, Sms.TEXT_MODE_STATUS_MAP[msgStatus], number, parseTextModeTimeStr(msgTime),
                                msgText, None, [], msgIndex))
                delMessages.add(int(msgIndex))
        else:
                ...
                if not readPdu:
                    ...
                    try:
                        ...
                        if smsDict['type'] == 'SMS-DELIVER':
                            sms = ReceivedSms(self, int(msgStat), smsDict['number'], smsDict['time'], smsDict['text'],
                                              smsDict['smsc'], smsDict.get('udh', []), msgIndex)
                        elif smsDict['type'] == 'SMS-STATUS-REPORT':
                            sms = StatusReport(self, int(msgStat), smsDict['reference'], smsDict['number'],
                                               smsDict['time'], smsDict['discharge'], smsDict['status'])
                        else:
    ...
    def __threadedHandleModemNotification(self, lines):
        ...
        for line in lines:
            if 'RING' in line:
                ...
                if self.cpinCallbackFunc:
                    self.cpinCallbackFunc(line)
                return
            else:
    ...
    def _handleSmsStatusReportTe(self, length, notificationLine):
        ...
        try:
            ...
            if smsDict['type'] == 'SMS-STATUS-REPORT':
                report = StatusReport(self, int(smsDict['status']), smsDict['reference'], smsDict['number'],
                                      smsDict['time'], smsDict['discharge'], smsDict['status'])
            else:
                raise CommandError('Invalid PDU type for readStoredSms(): {0}'.format(smsDict['type']))
        # Update sent SMS status if possible
    ...
    def readStoredSms(self, index, memory=None):
        ...
        if self.smsTextMode:
            ...
            if cmgrMatch:
                ...
                msgText = '\n'.join(msgData[1:-1])
                return ReceivedSms(self, Sms.TEXT_MODE_STATUS_MAP[msgStatus], number, parseTextModeTimeStr(msgTime),
                                   msgText)
            else:
                ...
                if cmgrMatch:
                    ...
                    return StatusReport(self, Sms.TEXT_MODE_STATUS_MAP[msgStatus], int(reference), number,
                                        parseTextModeTimeStr(sentTime), parseTextModeTimeStr(deliverTime),
                                        int(deliverStatus))
                else:
            ...
            if smsDict['type'] == 'SMS-DELIVER':
                return ReceivedSms(self, int(stat), smsDict['number'], smsDict['time'], smsDict['text'],
                                   smsDict['smsc'], smsDict.get('udh', []))
            elif smsDict['type'] == 'SMS-STATUS-REPORT':
                return StatusReport(self, int(stat), smsDict['reference'], smsDict['number'], smsDict['time'],
                ...
                if direction == 0:  # Outgoing call
                    ...
                    if expectedState == 0:  # waiting for call initiated
                        if stat == 2 or stat == 3:  # Dialing or ringing ("alerting")
                            ...
                            self._handleCallInitiated(None, callId,
                                                      callType)  # if self_dialEvent is None, this does nothing
